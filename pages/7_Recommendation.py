...
if op_margin > 0.20:
    fund_score += 1
    fund_notes.append(f"Strong operating margin ({op_margin * 100:.1f}%)")
else:
    fund_score -= 1
    fund_notes.append(f"Weak operating margin ({op_margin * 100:.1f}%)")
if roe > 0.15:
    fund_score += 1
    fund_notes.append(f"High ROE ({roe * 100:.1f}%) — moat proxy")
else:
    fund_notes.append(f"ROE below 15% ({roe * 100:.1f}%)")
if fcf > 0:
    fund_score += 1
    fund_notes.append(f"Positive FCF (${fcf / 1e9:.1f}B)")
else:
    fund_score -= 1
...
if trailing_eps > 0 and current_price > 0:
    ...
    if mos >= mos_threshold:
        val_score += 2
        val_notes.append(f"Attractive: {mos:.1f}% margin of safety vs base PE ({base_pe}x)")
    elif mos >= 0:
        val_score += 1
        val_notes.append(f"Fair: {mos:.1f}% margin of safety (below {mos_threshold}% threshold)")
    else:
        val_score -= 1
        val_notes.append(f"Stretched: {mos:.1f}% — stock trading above base PE fair value")
else:
...
if trailing_pe and trailing_pe > 0:
    if trailing_pe < 25:
        val_score += 1
        val_notes.append(f"Reasonable trailing P/E ({trailing_pe:.1f}x)")
    elif trailing_pe > 40:
        val_score -= 1
        val_notes.append(f"Elevated trailing P/E ({trailing_pe:.1f}x)")
    else:
...
if not hist.empty:
    ...
    if not math.isnan(sma50):
        if last_close > sma50:
            tech_score += 1
            tech_notes.append(f"Price above SMA50 (${sma50:.0f})")
        else:
            tech_score -= 1
            tech_notes.append(f"Price below SMA50 (${sma50:.0f})")
    if not math.isnan(sma200):
        if last_close > sma200:
            tech_score += 1
            tech_notes.append(f"Price above SMA200 (${sma200:.0f})")
        else:
            tech_score -= 1
            tech_notes.append(f"Price below SMA200 (${sma200:.0f})")
    if not math.isnan(rsi):
        if 40 < rsi < 65:
            tech_score += 1
            tech_notes.append(f"RSI in constructive zone ({rsi:.1f})")
        elif rsi >= 70:
            tech_score -= 1
            tech_notes.append(f"RSI overbought ({rsi:.1f})")
        else:
